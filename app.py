...
if auth_status == 1:
    ...
    with col[3]:
        ...
        centered_subheader = """  
        <div style="text-align:center; font-size:45px; font-weight:bold; color: rgb(255,75,75);">  
            标签仓储管理系统
        </div>  
        """  
            ...
            if st.button("登录", use_container_width=True,type='primary'):
                if username in credentials['usernames']:
                    if password == credentials['usernames'][username]['password']:
                        st.success("登录成功！")
                        st.session_state["auth_status"] = 2
                        st.session_state["auth_user"] = credentials['usernames'][username]['name']
                        st.rerun()
                    else:
                        st.error("用户名或密码错误！")
                        st.session_state["auth_status"] = 1
